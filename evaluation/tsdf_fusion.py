...
measure.marching_cubes_lewiner = measure.marching_cubes
...
def save_tsdf(args, scan, save_mesh=True, not_dtu=False):
    ...
    fnames = os.listdir(os.path.join(args.root_dir, "depth", scan))

    for id in views:
        if not_dtu:
            data_tmp = np.load(os.path.join(args.root_dir, "depth", scan, fnames[id]), allow_pickle=True).item()
        else:
            data_tmp = np.load(os.path.join(args.root_dir, "depth", scan, "{:0>8}.npy".format(id)), allow_pickle=True).item()
    ...
    if args.volume_bnd is not None:
        vol_bnds[:, 0] = - args.volume_bnd
        vol_bnds[:, 1] = args.volume_bnd
    ...
    for id in views:
        ...
        cam_intr = data_tmp['intrinsic']#[:3,:3]
        cam_pose = np.linalg.inv(data_tmp['extrinsic'])
        color_image = data_tmp['rgb'] if 'rgb' in data_tmp else None
        # color_image = data_tmp['rgb'].astype(np.float32) / 255 if 'rgb' in data_tmp else None
        # print(color_image.max(), color_image.min(), color_image.mean())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_scene', dest='test_scene', type=str, default=None)
    ...
    parser.add_argument('--get_point_cloud', action='store_true')
    parser.add_argument('--volume_bnd', default=None, type=float)
    ...
    if args.scan is None:
        assert args.test_scene is not None
        scans = [args.test_scene]
        not_dtu = True
    else:
        scans = ["scan%d"%args.scan]
        not_dtu = False
    ...
    for scan in scans:
        save_tsdf(args, scan, not_dtu=not_dtu)