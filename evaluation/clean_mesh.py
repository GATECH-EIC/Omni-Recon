...
def clean_mesh_faces_by_mask(args, mesh_file, new_mesh_file, scan, imgs_idx, minimal_vis=0, mask_dilated_size=11, trans2w_mat=None):
    old_mesh = trimesh.load(mesh_file)
    old_vertices = old_mesh.vertices[:]
    old_faces = old_mesh.faces[:]

    # print(old_vertices[:10])
    
    if trans2w_mat is not None:
        old_vertices_homo = np.concatenate((old_vertices, np.ones_like(old_vertices[...,:1])), axis=1)
        old_vertices_transform = np.matmul(trans2w_mat, old_vertices_homo.transpose()).transpose()[:,:3]
    else:
        old_vertices_transform = old_vertices
    
    # print(old_vertices_transform[:10])
    # input()
    
    mask = clean_points_by_mask(args, old_vertices_transform, scan, imgs_idx, minimal_vis, mask_dilated_size)
...
def clean_mesh_faces_outside_frustum(args, scan, old_mesh_file, new_mesh_file, imgs_idx, H=1200, W=1600, mask_dilated_size=11,
                                     isolated_face_num=500, keep_largest=True, trans2w_mat=None):
    ...
    if trans2w_mat is not None:
        old_vertices = mesh.vertices
        old_vertices_homo = np.concatenate((old_vertices, np.ones_like(old_vertices[...,:1])), axis=1)
        mesh.vertices = np.matmul(trans2w_mat, old_vertices_homo.transpose()).transpose()[:,:3]
...
if __name__ == "__main__":
    ...
    for scan in scans:
        ...
        # if os.path.exists(os.path.join(args.out_dir, "trans2w_mat.npy")):
        #     trans2w_mat = np.load(os.path.join(args.out_dir, "trans2w_mat.npy"))
        # else:
        #     trans2w_mat = None
        trans2w_mat = None  ## this transformation has been performed before saving the mesh

        clean_mesh_file = os.path.join(args.out_dir, "final", "clean_%d.ply" % scan)
        ...
        clean_mesh_faces_by_mask(args, old_mesh_file, clean_mesh_file, scan, imgs_idx, minimal_vis=1,
                                 mask_dilated_size=mask_kernel_size, trans2w_mat=trans2w_mat)
        clean_mesh_faces_outside_frustum(args, scan, clean_mesh_file, final_mesh_file, imgs_idx, mask_dilated_size=mask_kernel_size, trans2w_mat=trans2w_mat)
