...
class ResUNet(nn.Module):
    def __init__(self,
        ...
        self.dilation = 1
        block = BasicBlock if encoder in ['resnet18', 'resnet34'] else Bottleneck
...
class ResUNet_Multi_Res(nn.Module):
    def __init__(self,
                 encoder='resnet34',
                 out_ch=32,
                 norm_layer=None,
        ...
        super(ResUNet_Multi_Res, self).__init__()
        assert encoder in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152'], "Incorrect encoder type"
        if encoder in ['resnet18', 'resnet34']:
            filters = [64, 128, 256, 512]
        else:
        ...
        self.output_feat3 = nn.Conv2d(256, 24, 3, stride=1, padding=1)

        self.output_feat2 = nn.Conv2d(128, 16, 3, stride=1, padding=1)

        self.output_feat1 = nn.Conv2d(out_ch, 16, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        feat3_proj = self.output_feat3(x3)
        
        x = self.upconv3(x3)
        x = self.skipconnect(x2, x)
        x = self.iconv3(x)
        
        feat2_proj = self.output_feat2(x)
        
        x = self.upconv2(x)
        x = self.skipconnect(x1, x)
        x = self.iconv2(x)
        
        feat1_proj = self.output_feat1(x)
        
        x_out = self.out_conv(x)
        
        
        
        return x_out, [feat1_proj,feat2_proj,feat3_proj]