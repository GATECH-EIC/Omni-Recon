...
def get_ray_directions_blender(H, W, focal, center=None, supersampling: int = 1):
    """
    Get ray directions for all pixels in camera coordinate.
    Reference: https://www.scratchapixel.com/lessons/3d-basic-rendering/
               ray-tracing-generating-camera-rays/standard-coordinate-systems
    ...
    Outputs:
        directions: (H, W, 3), the direction of the rays in camera coordinate
    """
    grid = create_meshgrid(H*supersampling, W*supersampling, normalized_coordinates=False)[0]+0.5
    i, j = grid.unbind(-1)
    # the direction here is without +0.5 pixel centering as calibration is not so accurate
    # see https://github.com/bmild/nerf/issues/24
    cent = center if center is not None else [W*supersampling / 2, H*supersampling / 2]
    directions = torch.stack([(i - cent[0]) / (focal[0]*supersampling), -(j - cent[1]) / (focal[1]*supersampling), -torch.ones_like(i)],
                             -1)  # (H, W, 3)
