...
class LSegmentationModule(pl.LightningModule):
    ...
    def forward(self, x, labelset="", return_feature=False, input_feature=False):
        return self.net(x, labelset=labelset, return_feature=return_feature, input_feature=input_feature)

    def evaluate(self, x, target=None, return_feature=False, input_feature=False):
        pred = self.net.forward(x, return_feature=return_feature, input_feature=input_feature)
    ...
    def evaluate_random(self, x, labelset, target=None, return_feature=False, input_feature=False):
        pred = self.net.forward(x, labelset, return_feature=return_feature, input_feature=input_feature)
        if isinstance(pred, (tuple, list)):
            pred = pred[0]
        if target is None:
            return pred
