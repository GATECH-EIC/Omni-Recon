...
class LSeg_MultiEvalModule(DataParallel):
    ...
    def forward(self, image, label_set='', return_feature=False, input_feature=False):
        ...
        for scale in self.scales:
            ...
            if long_size <= crop_size:
                ...
                outputs = module_inference(self.module, pad_img, label_set, self.flip, return_feature=return_feature, input_feature=input_feature)
                ...
                for idh in range(h_grids):
                    for idw in range(w_grids):
                        ...
                        # pad if needed
                        if input_feature:
                            pad_crop_img = crop_img
                        else:
                            pad_crop_img = pad_image(crop_img, self.module.mean,
                                                    self.module.std, crop_size)
                        output = module_inference(self.module, pad_crop_img, label_set, self.flip, return_feature=return_feature, input_feature=input_feature)
...
def module_inference(module, image, label_set, flip=True, return_feature=False, input_feature=False):
    output = module.evaluate_random(image, label_set, return_feature=return_feature, input_feature=input_feature)
    if flip:
        fimg = flip_image(image)
        foutput = module.evaluate_random(fimg, label_set, return_feature=return_feature, input_feature=input_feature)
