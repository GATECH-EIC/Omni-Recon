...
class MVSDataset(Dataset):
    def __init__(self, root_dir, split, n_views=3, img_wh=(640, 512),
        ...
        self.no_warp_to_ref_view = no_warp_to_ref_view
        ...
        # self.test_ref_views = [0, 8, 16, 24, 32, 40, 48] 
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if self.no_warp_to_ref_view:
                w2cs.append(self.all_extrinsics[index_mat])
            else:
                w2cs.append(self.all_extrinsics[index_mat] @ w2c_ref_inv)   # the new "world" space is the camera space of the reference view
        ...
        if self.args.camera_correction:
            ...
            sample['ref_pose'] = (offset + normalize_matrix @ intrinsics_pad[0]) @ sample['w2cs'][0] 
            sample['source_poses'] = (offset + normalize_matrix @ intrinsics_pad[1:]) @ sample['w2cs'][1:] 
        else:
            sample['ref_pose'] = normalize_matrix @ intrinsics_pad[0] @ sample['w2cs'][0] 
            sample['source_poses'] = normalize_matrix @ intrinsics_pad[1:] @ sample['w2cs'][1:]
        ...
        # ray_o = sample['c2ws'][:,:3,-1] # (4, 3)
        # ray_z = sample['c2ws'][:,:3,2]  # (4, 3)
        ...
        # print('intrinsics:', sample['intrinsics'][0])
        # print('w2cs:', sample['w2cs'][0])
        # print('scale_mat:', sample['scale_mat'])
        # print('trans_mat:', sample['trans_mat'])
        # print('near_fars:', sample['near_fars'])
